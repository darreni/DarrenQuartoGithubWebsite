...
I thank Joshua French for his excellent Youtube guide which helped me with some of these steps: <https://www.youtube.com/watch?v=uimdXPZc40I&list=PLkrJrLs7xfbXcEKhTCKRSr2VXH4yiBeXo&index=4>
...
The above will create a set of files for a basic website. The home page is `index.qmd` and there is also an "about" page in `about.qmd`. There is also a The `_quarto.yml` file, which is a configuration file that provides website options and defaults for pages on the site. This can be opened and inspected in VS Code.
...
To set up to render and sync local folder to Github repo, then you still need to link the full local folder to the Github repo. One way to do this (worked for me) was: On VS Code welcome page, choose "Clone Git Repository", then choose the name of the Github repo, then choose the local project directory, then click on "Select as Repository Destination".

Then, the local folder is the same as the Github one, and any changes can be Synchronized.
